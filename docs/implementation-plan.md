...
[  ] フェーズ4  命令を1つずつ             Task 9-13
[  ] フェーズ5  既読・バックログ・セーブ  Task 14-17
[  ] フェーズ6  ページ送りと仕上げ        Task 18-20
```
...
**実装前にこれらを確定し、`engine-spec.md` / `architecture.md` を更新して
決定記録を1本足す**（status.md の書き分けルールに従う）。

以下は本計画が採用している案であり、これで進めてよいかの確認が要る。
...
- [ ] **Step 1: `src/engine/ui/Settings.tsx` を書く**
